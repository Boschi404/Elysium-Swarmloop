"""Elysium-Bench SUPER LUNGO — ~1 ora
10 categorie × 2 loop + Re-Test = 30 task
Skill: elysium-swarmloop v0.7.2"""
...
TOTAL_LOOPS = 2  # Loop 1 (measurement) + Loop 2 (practice) + Re-Test = 30 task
...
all_results = {"timestamp":time.strftime("%Y-%m-%dT%H:%M:%S"),
               ...
               "skill":f"{SKILL_NAME} v0.7.2","loops":{}}
start_all = time.time()
total_tasks = 0
...
for phase in range(1, TOTAL_LOOPS + 2):
    ...
    for cat_id in CATEGORIES:
        ...
        if result["mode"] == "timeout":
            ...
            print(f"→ {fmt(score.total)}/100 {'✅' if score.passed else '❌'} [{ts}] ⏱{result['elapsed']}s")
        ...
        done_ratio = total_tasks / (len(CATEGORIES)*(TOTAL_LOOPS+1))
        if done_ratio > 0:
            eta = (elapsed / done_ratio - elapsed) / 60
            print(f"     ⏳ {total_tasks}/{len(CATEGORIES)*(TOTAL_LOOPS+1)} task | +{elapsed/60:.1f}min | ETA ~{eta:.0f}min")
...
print(f"\n{'═'*60}")
print(f"  📊 FINAL REPORT — SUPER LUNGO")
print(f"{'═'*60}")
...
for cat_id, cd in cats_data.items():
    d = cd['delta']
    d_icon = "📈" if d > 0 else ("📉" if d < 0 else "➡️")
    print(f"  {cat_id:<30} {cd['loop1']:<8.1f} {cd['loop2']:<8.1f} {cd['retest']:<10.1f} {d:>+6.1f} {d_icon}")
...
print(f"  {'Task totali':<30} {total_tasks}")

report = {
    "benchmark": "Elysium-Bench SUPER LUNGO",
    "version": "1.0.0",
    "skill": f"{SKILL_NAME} v0.7.2",
    ...
    "avg_loop2": round(l2_avg, 1),
    "avg_retest": round(rt_avg, 1),
    "avg_delta": round(delta_avg, 1),
    "improvement_detected": delta_avg >= 5,
    "loops": all_results["loops"],
}

RISULTATI_DIR.mkdir(parents=True, exist_ok=True)
ts = time.strftime("%Y%m%d_%H%M%S")
jp = RISULTATI_DIR / f"superlungo_v072_{ts}.json"
jp.write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")

md = [
    f"# Elysium-Bench — SUPER LUNGO (Elysium Swarmloop v0.7.2)",
    f"**Durata:** {total_elapsed/60:.1f} min | **Task:** {total_tasks} | **Categorie:** {len(CATEGORIES)}",
    f"",
    f"## 📊 Progressione",
...
md += [
    f"",
    f"**Loop 1 Avg:** {l1_avg:.1f} | **Loop 2 Avg:** {l2_avg:.1f} | **Re-Test Avg:** {rt_avg:.1f} | **Δ:** {delta_avg:+.1f}",
    f"**Miglioramento:** {'✅ SI' if delta_avg >= 5 else '❌ NO'} ({delta_avg:+.1f})",
    f"",
    f"## ⏱ Dettaglio Task",
    f"| Phase | Task | Type | Score | Tests | Durata |",
    f"|-------|------|:---:|:----:|:-----:|:------:|",
]
for pk, scores in all_results["loops"].items():
    for tid, sd in scores.items():
        ...
        md.append(f"| {pk} | {tid} | {sd.get('task_type','?')} | {sd['total']:.1f} | {ts} | {sd['duration_s']}s |")

mp = RISULTATI_DIR / f"superlungo_v072_{ts}.md"
mp.write_text("\n".join(md), encoding="utf-8")
print(f"\n  📁 {jp.name}")
print(f"  📁 {mp.name}")

clean_ws()
print(f"\n  ✅ SUPER LUNGO completato!\n")
